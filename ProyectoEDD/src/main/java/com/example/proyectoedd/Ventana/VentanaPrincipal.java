...
public class VentanaPrincipal {
    ...
    /** Construye y devuelve el root de esta pantalla. */
    public Parent getView() {
        ...
        toolbar = new ToolbarView(service, new UiActions() {
            @Override public void onBuscarRuta()        { buscarRuta(); }
            @Override public void onAgregarAeropuerto() { agregarAeropuertoDialog(); }
            @Override public void onAgregarVuelo()      { agregarVueloDialog(); }
            @Override public void onEditarVuelo()       { editarVueloDialog(); }
            @Override public void onEliminarVuelo()     { eliminarVueloDialog(); }
            @Override public void onCargarCSV()         { cargarCSVDialog(); }
            @Override public void onGuardarCSV()        { guardarCSVDialog(); }
            @Override public void onConectarAleatorio() { conectarAleatorioDialog(); }
            @Override public void onModoRealista()      { service.aplicarModoRealista(); drawGraph(); setInfo("Modo realista aplicado."); }
            @Override public void onQuitarDirecto() {
                var o = toolbar.getCbOrigen().getValue();
                var d = toolbar.getCbDestino().getValue();
                if (o == null || d == null) { warn("Selecciona Origen y Destino."); return; }
            ...
            @Override public void onRedibujar()         { drawGraph(); }
    ...
    private void refreshCombos() {
        ...
        if (!aeropuertos.isEmpty()) toolbar.getCbDestino().getSelectionModel().select(aeropuertos.get(0));
    }

    private String currentUnit() {
        String sel = toolbar.getCbMetrica().getValue();
        if (sel == null) return "min";
        if (sel.startsWith("Tiempo"))    return "min";
        if (sel.startsWith("Distancia")) return "km";
        if (sel.startsWith("Costo"))     return "$";
        return "min";
    }

    // ====== Dibujo y lógica del grafo ======
    private void drawGraph() {
        ...
        for (Vertex<Aeropuerto, Vuelo> v : vertices) {
            if ("PKX".equalsIgnoreCase(v.getContent().getCodigo())) pkx = v; else others.add(v);
        }
        if (pkx == null) { pkx = vertices.get(0); others.clear(); for (int i=1;i<vertices.size();i++) others.add(vertices.get(i)); }
        ...
        for (var v : vertices) for (var e : v.getEdges()) { minW = Math.min(minW, e.getWeight()); maxW = Math.max(maxW, e.getWeight()); }
        if (minW == Integer.MAX_VALUE) { minW = 0; maxW = 1; }
        ...
        for (var v : vertices) {
            ...
            for (var e : v.getEdges()) {
                ...
                EdgeView ev = drawDirectedEdge(x1, y1, x2, y2, c, 1.8,
                        wgt + " " + currentUnit(), fromA.getCodigo(), toA.getCodigo());

                edgeViews.put(key(fromA.getCodigo(), toA.getCodigo()), ev);
            }
        }

        service.masConectadoSalida().ifPresent(a -> setInfo("Más conectado (salidas): " + a.getCodigo()));
    }
    ...
    private void placeAirportNode(Aeropuerto a, double x, double y, boolean central) {
        ...
        dot.setLayoutX(x); dot.setLayoutY(y);
        dot.setTranslateX(-r); dot.setTranslateY(-r);

        Tooltip.install(dot, new Tooltip(a.getCodigo() + (a.getNombre()!=null && !a.getNombre().isBlank() ? " - " + a.getNombre() : "")));
    ...
    private void makeDraggable(StackPane dot, String code, double r) {
        ...
        dot.setOnMousePressed(e -> { if (e.getButton()!=MouseButton.PRIMARY) return; start[0]=e.getSceneX(); start[1]=e.getSceneY(); });
    ...
    private void updateEdgesFor(String movedCode, double r) {
        edgeViews.entrySet().stream().filter(e -> e.getKey().startsWith(movedCode + "->")).forEach(e -> repositionEdge(e.getValue(), r));
        edgeViews.entrySet().stream().filter(e -> e.getKey().endsWith("->" + movedCode)).forEach(e -> repositionEdge(e.getValue(), r));
    }
    ...
    private EdgeView drawDirectedEdge(double x1, double y1, double x2, double y2,
        ...
        double mx = (sx + tx) / 2.0, my = (sy + ty) / 2.0;
        label.setLayoutX(mx + 6); label.setLayoutY(my - 6);

        gEdges.getChildren().addAll(line, arrow);
        gLabels.getChildren().add(label);

        ev.line = line; ev.arrow = arrow; ev.label = label;
        ev.from = fromCode; ev.to = toCode; ev.baseColor = color;
        return ev;
    }
    ...
    private void highlightIncident(String code, boolean on) {
        edgeViews.forEach((k, ev) -> {
            ...
            if (incident) {
                ...
                ev.label.setStyle(on ? "-fx-font-size: 12px; -fx-font-weight: bold; -fx-fill: #ffffff;"
    ...
    private void clearHighlights() { undimAllEdges(); }
    ...
    private void agregarAeropuertoDialog() {
        ...
        if (a.getCodigo() == null || a.getCodigo().length() != 3) { warn("El código debe tener 3 letras."); return; }
        if (service.buscarAeropuerto(a.getCodigo()) != null) { warn("Ya existe un aeropuerto con ese código."); return; }
    ...
    private void agregarVueloDialog() {
        var aeropuertos = service.listarAeropuertos();
        if (aeropuertos.size() < 2) { warn("No hay suficientes aeropuertos."); return; }
        ...
        if (aerolinea.isBlank()) { warn("Aerolínea requerida."); return; }

        var A = o.get(); var B = d.get();
        if (A.getCodigo().equalsIgnoreCase(B.getCodigo())) { warn("Origen y destino no pueden ser iguales."); return; }
        ...
        if (!ok) { error("No se pudo agregar el vuelo."); return; }
    ...
    private void editarVueloDialog() {
        ...
        var oSel = chO.showAndWait(); if (oSel.isEmpty()) return;

        ChoiceDialog<Aeropuerto> chD = new ChoiceDialog<>(toolbar.getCbDestino().getValue(), aeropuertos);
        chD.setTitle("Editar vuelo"); chD.setHeaderText("Seleccione DESTINO");
        var dSel = chD.showAndWait(); if (dSel.isEmpty()) return;
        ...
        if (!ok) { warn("No existe ese vuelo."); return; }
    ...
    private void conectarAleatorioDialog() {
        ...
        Slider slProb = new Slider(0.0, 1.0, 0.3); slProb.setShowTickMarks(true); slProb.setShowTickLabels(true); slProb.setMajorTickUnit(0.25);
        ...
        var r = dlg.showAndWait(); if (r.isEmpty()) return;
    ...
    // ====== Utils ======
    private void setInfo(String s) { lblInfo.setText(s); }
    private void setInfoSafe(String s) { System.out.println(s); }
    private void warn(String s) { new Alert(Alert.AlertType.WARNING, s).showAndWait(); }
    private void error(String s) { new Alert(Alert.AlertType.ERROR, s).showAndWait(); }

    private static double clamp(double v, double a, double b) { return Math.max(a, Math.min(b, v)); }
    private static double normalize(double v, double min, double max) { if (max <= min) return 0.0; return (v - min) / (max - min); }
